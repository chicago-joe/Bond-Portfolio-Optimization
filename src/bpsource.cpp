...
void get_optimal_portfolio()
{
    ...
    else
    {
        ...
        cout << "Largest convexity we can get is: " << get_objective(lp) << endl << "Optimal portfolio (%): " << endl;
        for (int i = 0; i < bonds; i++)
        {
            cout << "Bond " << i + 1 << ": " << solution[i] << endl << endl;
        }

        cout << "To create optimal portfolio: BUY " << endl;
        for (int i = 0; i < bonds; i++)
            if (solution[i] > 0)
